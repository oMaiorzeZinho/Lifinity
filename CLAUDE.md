...
- [2026-06-25] Web — **Recuperação de palavra-passe por código (email)**: fluxo "esqueci-me da palavra-passe" no site.
  - **BD:** nova tabela `PASSWORD_RESET` (idreset, iduser→USER CASCADE, code, expires_at, created_at) no `estrutura_lifinity.sql` + ficheiro `docs/base_dados/password_reset.sql` (CREATE IF NOT EXISTS) para correr **uma vez** no phpMyAdmin (editar o .sql não cria a tabela no XAMPP a correr).
  - **Backend (3 rotas em `/api/auth`, estilo do `/login`):** `forgot-password` (procura email; **404** "Não existe nenhuma conta com esse email." se não existir; senão gera **código de 6 dígitos**, apaga os antigos do user, insere com `expires_at = NOW()+10min` e envia por email via `sendEmail`), `verify-reset-code` (valida email+code+`expires_at>NOW()`, **não** apaga), `reset-password` (revalida, valida nova pw ≥6, **hash bcryptjs** `genSalt(10)`, `UPDATE USER`, **apaga** o código — uso único). Prepared statements (`db.execute` com `?`), tempos calculados pelo MySQL. Helper `findValidReset`.
  - **Frontend:** link discreto "Recuperar palavra-passe" no `Login.jsx` (ao lado de "Contacte-nos") abre `PasswordResetModal.jsx` (novo) — modal de **3 passos** (email → código 6 dígitos → nova pw 2×) com loading, erros claros, reenviar código e passo X/3. Sucesso fecha o modal e mostra mensagem verde no login. Estilo **clay**: overlay com classe `lifinity-page` (os tokens clay estão scoped a essa classe; o Login não é uma) + fundo translúcido.
  - Validado: `node --check` + carregamento do `authRoutes` (3 handlers existem) **sem enviar emails**; `npm run build` do frontend OK. Documentado em `docs/estudo/RECUPERACAO_PALAVRA_PASSE_2026-06-25.md`.
- [2026-06-25] Web — **Nova palavra-passe não pode ser igual à atual** (validação cirúrgica, via `bcrypt.compare`), em dois sítios:
  - **Perfil** (`userController.updatePassword`): depois de confirmar a password atual, compara a nova com o hash atual (`bcrypt.compare(newPassword, users[0].password)`); se igual → `400 "A nova palavra-passe não pode ser igual à atual."` (antes do hash/UPDATE).
  - **Recuperação** (`authController.resetPassword`): depois de validar o código e o tamanho, busca o `password` atual do `USER` por `iduser` (o `findValidReset` não o traz) e compara; se igual → `400`, **sem apagar o código** (o utilizador pode tentar outra password com o mesmo código ainda válido).
  - Não foi preciso mexer no frontend (o modal de recuperação e as configurações do perfil já mostram a mensagem de erro do backend). Validado com `node --check` + carregamento dos controllers. Documentado em `RECUPERACAO_PALAVRA_PASSE_2026-06-25.md` e no doc do `userController`.
- [2026-06-26] Android — **Melhorias visuais na barra de navegação e no Perfil** (só UI/layout; sem tocar em lógica de negócio, rede ou navegação):
  - **Ícones do menu coerentes:** as 4 tabs usavam caracteres/emojis misturados (`☰`, o emoji a cores `👥`, `✦`, `○`). Substituídos por **vector drawables Material** desenhados à mão (`ic_nav_tasks`=checklist, `ic_nav_community`=group, `ic_nav_inspiration`=star, `ic_nav_profile`=person) — mesma família/peso, **sem dependências externas** (build robusta). `BottomNavHelper` recolore os ícones (`ImageView.setColorFilter`) por estado em vez de `setTextColor`.
  - **Miniatura do avatar na tab Perfil:** a tab Perfil passou a mostrar a **foto real** (circular) — `FrameLayout` com ícone `person` (fallback) + `ImageView` `navTabProfileAvatar` por cima, carregada pelo `AvatarLoader`/`ImageUrlHelper` (avatar lido das `SharedPreferences`, como no `ProfileActivity`). Sem foto → fica o `person`. Aparece em todas as telas (o `BottomNavHelper` corre em todas).
  - **FAB "+" que não aparecia — CAUSA REAL:** todos os `<include>` do `nav_bottom` fixavam `layout_height="wrap_content"`, o que **anulava** os 96dp do FrameLayout (colapsava para ~70dp) e o FAB deixava de emergir acima da barra. Corrigido com `@dimen/nav_total_height` (96dp) usado **no `nav_bottom` e nos 5 `<include>`** (tasks/profile/ranking/inspiration/community). O FAB volta a emergir ~26dp acima da barra, visível e clicável. (As tentativas anteriores — margem negativa, FrameLayout 96dp — estavam corretas no `nav_bottom` mas eram anuladas pelo `<include>`.)
  - **Cabeçalho das Tarefas sobreposto ao scroll:** o cabeçalho fixo era transparente, por isso o conteúdo deslizava por trás da marca e os textos pareciam colidir. Dado **fundo opaco** (`@color/lifinity_bg`) + `elevation="6dp"` + respiro acima do título "Atividades" (`marginTop` 12dp) e `paddingBottom` 12→14dp. Mesmo tratamento aplicado ao cabeçalho do Perfil (coerência).
  - **"Mais no Lifinity" reorganizado:** removida a secção redundante (Notificações/Conquistas/Definições) — Notificações já está no sino (`HeaderHelper`) e Definições na engrenagem (`headerSettingsIcon`). As Conquistas passaram a ter um botão **"Ver todas ›"** ao lado do título da secção CONQUISTAS (abre o mesmo `AchievementsActivity`). `ProfileActivity` ajustado: removidos os 3 listeners órfãos, ligado o novo botão.
  - Documentado em `docs/estudo/android/MELHORIAS_NAV_E_PERFIL_2026-06-26.md`. Validado com `gradlew :app:assembleDebug` (BUILD SUCCESSFUL).
- [2026-06-26] Android — **4 melhorias: FAB "+", barra de XP, foto de perfil e destaque de conquistas** (tema claro mantido; lógica de negócio e endpoints inalterados — só *chamados*):
  - **FAB "+" garantido** (`nav_bottom.xml`, `ic_add.xml` novo): o `<Button>` era trocado pelo tema por um `MaterialButton` (insets/handling do texto próprios) e o "+" não renderizava. Trocado por um **`ImageView`** (que o inflater Material não substitui) com o "+" como **vector drawable** branco centrado (`scaleType=fitCenter`, padding 16dp), mantendo `bg_fab_clay` e a elevação. O onClick mantém-se (o `BottomNavHelper` trata o FAB como `View`).
  - **Barra de XP de "jogo"** (`xp_progress_bar.xml` novo, `activity_profile.xml`, `ProfileActivity`): `progressDrawable` em `layer-list` (track claro arredondado + preenchimento verde com gradiente, recortado por `<clip>`); `maxHeight/minHeight=16dp` **essenciais** (o ProgressBar horizontal limita a altura pelo `maxHeight`). **Cálculo:** a fórmula já era igual ao backend; o bug de "barra cheia" vinha de confiar no `level` guardado (dessincronizado) → passou a **derivar o nível do XP** (`calculateLevelFromXp`) quando há XP. Label agora com percentagem: "X XP para nível Y  (Z%)".
  - **Mudar foto de perfil pela galeria** (`ic_camera.xml`/`bg_avatar_edit_badge.xml` novos, `activity_profile.xml`, `UserApi`, `ProfileActivity`): selo de câmara sobreposto ao avatar abre o **photo picker** (`ActivityResultContracts.GetContent` + `launch("image/*")`, **sem permissões perigosas**). Lê os bytes do `Uri` via `ContentResolver`, valida no cliente (≤2MB, jpeg/png/webp) e envia multipart (campo `image`) para o endpoint **já existente** `PUT /users/me/avatar`. No sucesso atualiza a foto no cartão, a miniatura da barra e o `avatar` nas `SharedPreferences` (Toast de feedback).
  - **Destacar conquistas** (`item_achievement.xml`, `AchievementAdapter`, `AchievementApi`, `AchievementsActivity`): nas conquistas **desbloqueadas**, botão **ghost** "Destacar"/"Remover destaque" (`btn_ghost_clay`), visualmente distinto do **selo verde sólido** "★ Destacada". Lógica: <3 destaques → adiciona; já 3 → **diálogo** (tema claro) para escolher qual substituir; já destacada → remove (toggle). Posições reatribuídas 1..n e `PUT /achievements/highlights` (já existente). Recarrega a lista; o Perfil relê os destaques no `onResume`.
  - Documentado em `docs/estudo/android/FAB_BARRA_XP_FOTO_PERFIL_DESTAQUES_2026-06-26.md` (+ índice). Validado com `gradlew :app:assembleDebug` (BUILD SUCCESSFUL).
- [2026-06-26] Android — **Página de Inspiração: 3 melhorias (banner, filtro de temas, partilha)** (tema claro mantido; lógica e endpoints inalterados — só *chamados*):
  - **Imagem de fundo atrás do versículo do dia** (`bible_banner.jpg` copiada da web `frontend/public/images/bible-banner.jpg`; `bg_verse_hero.xml`/`bg_verse_overlay.xml` novos; `activity_inspiration.xml`): o cartão do versículo ganhou um **banner** (`FrameLayout` com imagem `centerCrop` + overlay escuro em gradiente + conteúdo por cima). Cantos arredondados via `clipToOutline=true` + fundo `bg_verse_hero` com cantos **uniformes** (22dp, recorte fiável em API 24+). Textos passaram a **branco** (com sombra no versículo) para legibilidade; cores só no XML (o Java não as sobrepõe). Botões mantidos por baixo do banner, na zona clay. O fundo geral da página continua claro.
  - **Filtro por tema nos favoritos** (`activity_inspiration.xml`, `InspirationActivity`): `Spinner` (estilo do seletor das Estatísticas, `item_spinner`) com `"Todos"` + **temas únicos derivados dinamicamente** dos favoritos (campo `Verse.getTheme()`); favoritos sem tema são ignorados e o filtro esconde-se se não houver temas. `renderFavorites()` separa carregar de mostrar; flag `suppressThemeSpinnerEvent` evita render duplicado ao repovoar o Spinner. Mensagens distintas para "sem favoritos" vs "nenhum favorito neste tema".
  - **Partilhar versículo para conversa EXISTENTE** (`ic_share.xml` novo; `activity_inspiration.xml`, `InspirationActivity`): ícone de partilha no versículo do dia + botão "Partilhar" (ghost) em cada favorito. Carrega `GET chat/conversations` (Toast de loading), abre **`AlertDialog`** (tema claro) com os nomes; ao escolher envia `POST chat/conversations/{id}/messages` com conteúdo «"texto" — Livro cap:verso» (`SendChatMessageRequest` só tem `content`). **Só conversas existentes** — não cria conversas nem lista amigos/grupos; se não houver conversas, mostra "Ainda não tens conversas para partilhar.". Feedback "Versículo enviado.".
  - Documentado em `docs/estudo/android/INSPIRACAO_BANNER_FILTRO_PARTILHA_2026-06-26.md` (+ índice). Validado com `gradlew :app:assembleDebug` (BUILD SUCCESSFUL).
- [2026-06-27] **Correções do chat e melhorias sociais** (Android + web; backend só ganhou 1 campo num `SELECT`):
  - **(CRÍTICA) Mensagens no lado errado — chat E assistente.** Causa real: **campos mal mapeados no GSON**, não o `==`. O backend devolve o remetente em `idsender` (não `iduser`) e `sender` (não `role`), por isso o campo vinha `null` ao recarregar e tudo caía no lado "recebido"/IA. Corrigido com `@SerializedName(value="idsender", alternate={"iduser"})` em `ChatMessage` e `@SerializedName(value="sender", alternate={"role"})` em `AssistantMessage`; `isMine()` passou a comparar por **valor** (`iduser.intValue() == currentUserId`). Bónus: a resposta da IA vem em `reply` (não `assistantMessage`) → `AssistantSendResponse` mapeia `@SerializedName(value="reply", alternate={"assistantMessage"})` (antes a IA caía sempre no texto de fallback).
  - **Nome do amigo nas conversas privadas** (mostrava "Conversa"): `Conversation` ganhou `type/idgroup/other_user_id/other_username/other_avatar/last_sender_username` + `getDisplayName()`/`getDisplayAvatar()` (grupos→`name`; privadas→`other_username`). Aplicado na **lista** (`ConversationAdapter`) e no **cabeçalho** (`ConversationsActivity` passa o nome já resolvido).
  - **Foto na lista de conversas (Android):** `+ other_user.avatar AS other_avatar` em `chatController.getConversations` (única alteração ao backend, serve Android+web). `item_conversation.xml` ganhou avatar (FrameLayout placeholder+ImageView); `ConversationAdapter` usa `AvatarLoader` (privadas→foto, grupos→placeholder com inicial).
  - **Nome do remetente dentro do chat:** `ChatMessage` ganhou `sender_username` (`getSenderName()`); `item_chat_received.xml` ganhou `chatSenderName`; `ChatMessageAdapter` mostra o nome só nas **recebidas** e só na 1.ª de uma sequência do mesmo remetente (enviadas não mostram nome). Layout usado é `item_chat_received.xml` (o `item_chat_message_received.xml` é duplicado não usado).
  - **Amigos: menu de 3 opções** ("•••" estilo tasks): **Abrir conversa** (`POST chat/conversations/private {idfriend}` → `idconversation` → `ChatActivity`), **Ver perfil** (diálogo simples com nome/nível/XP — TODO: ecrã de perfil público dedicado, a web usa `PublicProfileModal`) e **Remover** (lógica existente). `FriendAdapter` ganhou 2.º modo (opções) mantendo o modo "Adicionar" para a pesquisa; novos `CreatePrivateConversationRequest` + método no `ChatApi`.
  - **Foto nas conversas (web):** `Chat.jsx` importa `getImageUrl` e usa novo `ConversationAvatar` (foto do outro / ícone de grupo / inicial) no cabeçalho da conversa aberta e na lista, com o `other_avatar` novo.
  - Documentado em `docs/estudo/CORRECOES_CHAT_E_SOCIAL_2026-06-27.md` (+ índice). Validado: Android `gradlew :app:assembleDebug` (BUILD SUCCESSFUL), backend `node --check`, frontend `npm run build`.
- [2026-06-27] Android — **Inspiração: 3 retoques visuais** (cirúrgicos; tema claro e lógica mantidos):
  - **Banner do versículo mais baixo:** o cartão/imagem (`bible_banner`) atrás do versículo do dia ocupava ecrã a mais. Reduzido para uma **faixa contida** mantendo `wrap_content` (não corta versículos longos): `inspirationVerseHero` `minHeight` 200→**150dp**, padding do conteúdo 20→16dp, aspas decorativas 56→**34sp** (margem 12→4dp), texto do versículo 22→**18sp**, referência marginTop 16→10dp. Cantos arredondados, overlay e texto branco mantidos.
  - **Ícone de copiar reconhecível:** o `📋` não representava "copiar". Novo `res/drawable/ic_copy.xml` (vector à mão, **dois retângulos sobrepostos** = Material `content_copy`, `tint=lifinity_text` como o `ic_share`). O `copyVerseButton` passou de `<Button>`(texto) a **`<ImageView>`** com o mesmo estilo do botão de partilhar ao lado; no Java o campo mudou de `Button`→`ImageView` (onClick/copiar inalterados).
  - **Favoritos com "Mostrar mais/menos":** mostra só os **4 mais recentes** por defeito (o backend `getFavorites` já ordena por `created_at DESC` → basta cortar os primeiros); botão **ghost** "Mostrar mais (N)" expande para todos e "Mostrar menos" colapsa. **Coerente com o filtro de tema:** o corte aplica-se à lista já filtrada, e trocar de tema recomeça colapsado (`favoritesExpanded=false`). Em `renderFavorites()` + novo `createToggleFavoritesButton()` (sem XML novo).
  - Documentado em `docs/estudo/android/INSPIRACAO_RETOQUES_2026-06-27.md` (+ índice). Validado com `gradlew :app:assembleDebug` (BUILD SUCCESSFUL).
- [2026-06-27] Android — **Inspiração/Comunidade: banner gigante + cabeçalhos sobrepostos** (só XML, cirúrgico; tema claro mantido):
  - **Banner do versículo finalmente pequeno (causa real):** os retoques anteriores (reduzir `minHeight`/tamanhos) não chegaram porque o problema era estrutural — o `FrameLayout @id/inspirationVerseHero` em `wrap_content` com a `ImageView` de fundo (`bible_banner`) em `match_parent`: numa parent `wrap_content`, a imagem grande **infla a medição** e estica a faixa muito além do texto (aparecia um grande bloco de imagem por baixo). Solução fiável: **altura FIXA** `android:layout_height="200dp"` (removido o `minHeight`) — a `ImageView` `match_parent` passa a preencher exatamente 200dp em `centerCrop` e o texto fica por cima, faixa compacta. Cantos/overlay/texto branco e botões por baixo mantidos. (Versículos muito longos podem cortar ligeiramente — aceitável.)
  - **Cabeçalhos da Inspiração e da Comunidade sobrepostos:** faltava `android:background`+`android:elevation`, por isso o conteúdo do `NestedScrollView` desenhava-se **por cima** do cabeçalho e o título colidia com o logo/wordmark. Adicionado `android:background="@color/lifinity_bg"` + `android:elevation="6dp"` ao `LinearLayout` do HEADER em `activity_inspiration.xml` e `activity_community.xml` (igual ao Perfil/Tarefas; estrutura inalterada).
  - Documentado em `docs/estudo/android/CABECALHOS_E_BANNER_2026-06-27.md` (+ índice). Validado com `gradlew :app:assembleDebug` (BUILD SUCCESSFUL).
- [2026-06-27] Android — **Wordmark, botão "•••" e "X para Y" nas tarefas** (Android only; backend NÃO tocado — só se passou a ler campos já existentes):
  - **Wordmark "Lifinity" partia em 2 linhas** no cabeçalho das Tarefas (competia por espaço com a pill/ícones). Corrigido com `android:maxLines="1"` + `android:singleLine="true"` (+`ellipsize="end"`) no `TextView` do wordmark (`activity_tasks.xml`).
  - **Botão de opções "•••" aparecia como quadrado verde VAZIO** (Tarefas e Amigos) — **mesma causa do FAB**: `<Button android:text="•••">` é convertido em `MaterialButton` (fundo verde + texto próprio → pontos somem). Corrigido com a abordagem do FAB: novo vector `res/drawable/ic_dots.xml` (três pontos, `fillColor=lifinity_text`) e os botões passaram de `<Button>` a **`<ImageView>`** (mesmo id/tamanho/background, `src=ic_dots`, `scaleType=fitCenter`, padding, clickable) em `item_task.xml` (`taskOptionsButton`, 48dp, padding 12dp) e `item_friend.xml` (`friendOptionsButton`, 44dp, padding 11dp, mantém `visibility=gone`). Nos adapters (`TaskAdapter`, `FriendAdapter`) o campo `optionsButton` mudou de `Button`→`View` (evita `ClassCastException`); onClick/visibilidade e os menus mantêm-se.
  - **"X para Y" nos cartões de tarefa:** `models/Task.java` ganhou `creator_username`/`assignee_names`/`group_names`/`task_origin` (`@SerializedName`, já devolvidos pelo `taskController`) + helper `getCreatorAssigneeLabel()` (destinatário = `assignee_names` ou senão `group_names`; pessoal/sem destinatário ou criador==destinatário → null/esconde). Novo `TextView` `taskCreatorAssigneeText` (discreto) em `item_task.xml`; `TaskAdapter` preenche-o ou põe `GONE`. Ex.: "teste para cliente", "teste para Grupo Sigma"; tarefas pessoais não mostram a linha.
  - Documentado em `docs/estudo/android/WORDMARK_DOTS_E_X_PARA_Y_2026-06-27.md` (+ índice). Validado com `gradlew :app:assembleDebug` (BUILD SUCCESSFUL).
- [2026-06-29] Android — **6 retoques: cabeçalho/resumo/cartão XP das Tarefas, banner da Inspiração, atribuir tarefas a amigos/grupos e placeholder de prioridade** (tema claro mantido; **backend NÃO tocado** — as tarefas 5 e 6 só usam campos/endpoints já existentes):
  - **(T1) Pill removida do cabeçalho das Tarefas:** a pill `headerUserPill` ("utilizador · NÍV X") apertava o espaço e o wordmark "Lifinity" cortava ("Lifin…"). Removida por completo do `activity_tasks.xml` (o nível continua no cartão XP logo abaixo). **Obrigatório no Java:** removidas as 3 referências a `headerUserPill` em `TasksActivity` (campo, `findViewById`, bloco `setText` em `bindUserHeader`) — sem isto dava **`NullPointerException`** ao abrir o ecrã.
  - **(T2) Labels do "Resumo de hoje" numa linha:** "CONCLUÍDAS" quebrava em 2 linhas. Nos três labels (PENDENTES/CONCLUÍDAS/PERDIDAS): removido `letterSpacing`, `singleLine`+`maxLines=1`, `width=match_parent`+`gravity=center` e **autosize do AppCompat** (`app:autoSizeTextType=uniform`, 8→10sp) — encolhe só o necessário em vez de quebrar. Acrescentado `xmlns:app` à raiz; padding dos mini-cartões 12→10dp.
  - **(T3) Cartão "NÍVEL · XP" redesenhado:** novo drawable `bg_level_badge.xml` (retângulo arredondado, gradiente verde) como **badge** do nível (72×72dp, "NÍVEL"+número em branco) à esquerda; à direita XP em destaque + barra que passou a usar o `xp_progress_bar` (drawable que **já existia mas não estava a ser usado** nas Tarefas) com `min/maxHeight=16dp`. **Ids mantidos** (`xpCardLevelNumber/xpCardXpNumber/xpCardProgressBar/xpCardProgressLabel`) → `TasksActivity.java` inalterado.
  - **(T4) Banner do versículo (Inspiração) um pouco maior:** `inspirationVerseHero` 200dp→**250dp** (só XML) para a **referência** dos versículos mais longos (ex.: João 14:27) deixar de cortar.
  - **(T5) Criar tarefa: atribuir a amigos/grupos (igual ao browser):** `CreateTaskRequest` ganhou `List<Integer> assignees`/`groups` (nomes exatos p/ GSON; o `taskController` **já** os aceitava e valida amigos aceites/grupos próprios). Nova secção "DESTINO" em `activity_create_task.xml` (só `LinearLayout`/`TextView` clicáveis — **nada de `<Button>`**, que o tema converteria em `MaterialButton`): "Só para mim" + cabeçalhos expansíveis "Amigos"/"Grupos". `CreateTaskActivity` carrega `FriendApi.getFriends`/`GroupApi.getMyGroups`, cria as linhas em runtime, gere seleção (`Set` + repintar com `bg_pill_mint`/`bg_card_soft_clay`), contadores nos cabeçalhos e envia as listas (vazias = só para mim).
  - **(T6) Prioridade com placeholder "Prioridade":** spinner = `["Prioridade","baixa","media","alta"]`, `setSelection(0)`, `ArrayAdapter` custom que pinta a posição 0 com cor de hint; no `createTask()`, posição 0 → envia **"media"** (o backend rejeita valores fora de baixa/media/alta). O `R.array.task_priorities` deixou de ser usado neste ecrã.
  - Documentado em `docs/estudo/android/RETOQUES_TAREFAS_E_INSPIRACAO_2026-06-29.md` (+ docs por ficheiro e índice). Validado com `gradlew :app:assembleDebug` (BUILD SUCCESSFUL).
- [2026-07-01] Android — **7 correções: resumo por data, chatbot, concluir, tarefa perdida, pedidos e perfil de amigo** (tema claro mantido; **backend NÃO tocado** — todas usam campos/endpoints já existentes):
  - **(T1, funcional) "Resumo de hoje" por data:** o cartão mostrava concluídas/perdidas de SEMPRE (ex.: 12/8). `Task` ganhou `completed_at` (`@SerializedName`, já vinha do backend); `TasksActivity.updateSummary()` passou a contar CONCLUÍDAS só se concluídas hoje (`completed_at`) e PERDIDAS só se o prazo é hoje (`due_date`) — PENDENTES continuam TODAS. Novo helper `isSameDayAsToday()` (usa o `parseDate` existente + `Calendar`); concluídas/perdidas de outro dia não entram em nenhuma das três.
  - **(T2, visual) Banner da Inspiração:** `inspirationVerseHero` 250→**300dp** + texto do versículo 18→16sp + aspas 34→28sp, para caberem versículos longos (ex.: Jeremias 29:11) sem cortar. Só XML.
  - **(T3, funcional) Chatbot não respondia:** o backend (`assistantController`) lê `req.body.content`, mas o Android enviava `{ message }` → 400 → fallback. `AssistantSendRequest` mudou o campo `message`→**`content`** (o GSON serializa pelo nome do campo); `AssistantActivity` inalterado.
  - **(T4, funcional) Esconder "Concluir" em tarefas que criei para outra pessoa:** davam 403 ("só o destinatário conclui"). `Task` ganhou `has_assignees`/`has_groups` (0/1, já vinham do backend); `TaskAdapter.canCompleteTask()` esconde o "Concluir" se a tarefa tem destinatários individuais e eu não sou o destinatário (`task_origin != "assigned_to_me"`). Grupos e tarefas atribuídas a mim mantêm o "Concluir".
  - **(T5, visual) Tarefa perdida a coral:** novo `bg_card_lost.xml` (espelha `bg_card_clay` mas coral claro `#FCEAE5` + borda `lifinity_coral`); `TaskAdapter.bind()` define o fundo do cartão sempre (perdida→`bg_card_lost`, senão `bg_card_clay`, por causa da reciclagem) e `bindStatus()` mostra "PERDIDA" como rótulo vermelho (sem chip). Toda a info da tarefa mantém-se visível.
  - **(T6, visual) Botões aceitar/recusar de pedidos vazios:** mesmo bug do FAB/••• (`<Button>`→`MaterialButton` some o símbolo). Novos vectores `ic_check.xml`/`ic_close.xml`; em `item_friend_request.xml` os dois `<Button>` passaram a `<ImageView>` (mesmos ids, 44dp); `FriendRequestAdapter` mudou os campos `Button`→`View` (evita `ClassCastException`).
  - **(T7, visual) Perfil de amigo rico:** "Ver perfil" abria um popup só com "Nível X · Y XP". Novo model `PublicProfile` (reutiliza `Achievement`), endpoint `getPublicProfile` em `UserApi` (`GET /users/:id/public-profile`, já existia), layout `dialog_friend_profile.xml` e `FriendsActivity.showFriendProfile()` reescrito: avatar real (`AvatarLoader`) + nome + "Nível X · Y XP" (XP do `Friend`, o endpoint não devolve xp) + até 3 conquistas em destaque; "Sem conquistas em destaque." se vazio/erro.
  - Documentado em `docs/estudo/android/CORRECOES_RESUMO_CHATBOT_PERFIL_2026-07-01.md` (+ docs por ficheiro e índice). Validado com `gradlew :app:assembleDebug` (BUILD SUCCESSFUL).
- [2026-07-02] Android — **Botão "Ocultar concluídas/perdidas" nas Tarefas** (filtro de visualização; **backend NÃO tocado**):
  - **Chip nas Tarefas** (`tasksToggleHiddenButton`, `TextView` clicável — não `<Button>`) por baixo de "AS MINHAS ATIVIDADES", alinhado à direita. Alterna entre "Ocultar concluídas/perdidas" (cinzento) e "Mostrar todas" (menta).
  - **É um FILTRO DE VISUALIZAÇÃO client-side, NÃO o endpoint do backend** (esse arquiva permanentemente e só faz concluídas). Um `continue` no início do loop de `applyFilters()` esconde concluídas/perdidas **só de `filteredTasks`** (a lista visível); `allTasks` e `updateSummary()` ficam **intactos** → o "Resumo de hoje" (concluídas/perdidas de hoje) e o total de perdidas das Estatísticas continuam a contar as ocultas. **Nada é eliminado nem arquivado**; é reversível.
  - **Persistência:** flag `hideCompletedAndLost` + preferência `hide_completed_lost` nas `SharedPreferences` (lembrada entre sessões). `setupHideToggle()` lê/guarda e liga o clique; `updateToggleHiddenButton()` troca texto/fundo/cor (reaplica padding após `setBackgroundResource`). O chip só aparece na Lista (escondido no calendário, via `updateViewModeToggle`).
  - **Diferença face à web:** a web arquiva permanentemente só as concluídas; o Android é um toggle reversível que oculta concluídas **e** perdidas.
  - **Limpeza:** removido o campo morto `hideCompletedVisibleTasksCall` (e o `cancel` no `onDestroy`), que nunca era usado.
  - Documentado em `docs/estudo/android/OCULTAR_CONCLUIDAS_PERDIDAS_2026-07-02.md` (+ `TasksActivity.java.md` e índice). Validado com `gradlew :app:assembleDebug` (BUILD SUCCESSFUL).