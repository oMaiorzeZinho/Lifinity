...
- [RankingActivity](android/RankingActivity.java.md) — pódio + lista por XP · [NotificationsActivity](android/NotificationsActivity.java.md) — sino/lista (optimistic UI) · [CommunityActivity](android/CommunityActivity.java.md) — hub social · [FriendsActivity](android/FriendsActivity.java.md) — amigos (pesquisar, pedidos, perfil rico) · [SettingsActivity](android/SettingsActivity.java.md) — username/password/apagar conta
...
### Melhorias por data
- [**Botão "Ocultar concluídas/perdidas" nas Tarefas — 2026-07-02**](android/OCULTAR_CONCLUIDAS_PERDIDAS_2026-07-02.md) — chip nas Tarefas que **esconde da lista** as concluídas/perdidas (como no browser), mas como **filtro de visualização** client-side: um `continue` em `applyFilters` (só afeta `filteredTasks`), **sem** tocar em `allTasks`/`updateSummary` (o "Resumo de hoje" e as Estatísticas continuam a contá-las). **Toggle reversível** (oculta concluídas **e** perdidas), lembrado em `SharedPreferences`; **nada é eliminado nem arquivado** (não usa o endpoint do backend, que arquiva permanentemente só as concluídas).
- [**Resumo por data, chatbot, perfil de amigo e retoques — 2026-07-01**](android/CORRECOES_RESUMO_CHATBOT_PERFIL_2026-07-01.md) — sete correções: **resumo de hoje** (concluídas/perdidas contam só HOJE via `completed_at`; pendentes intactas), **banner da Inspiração** 250→300dp + texto menor (versículos longos como Jeremias 29:11), **chatbot** a responder (campo do pedido `message`→**`content`**, que o backend lê), esconder **"Concluir"** nas tarefas que criei para outra pessoa (`has_assignees`/`task_origin`, evita o 403), **cartão de tarefa perdida a coral** (`bg_card_lost` + rótulo "PERDIDA"), **botões aceitar/recusar** de pedidos `Button`→`ImageView`+vector (`ic_check`/`ic_close`) e **perfil de amigo rico** (`GET /users/:id/public-profile` → avatar + nível + XP + 3 conquistas). **Backend inalterado.**
- [**Retoques nas Tarefas/Inspiração + atribuir tarefas a amigos/grupos — 2026-06-29**](android/RETOQUES_TAREFAS_E_INSPIRACAO_2026-06-29.md) — seis retoques no tema claro: **pill removida** do cabeçalho das Tarefas (corrigia o "Lifinity" cortado; também se tiraram as 3 referências `headerUserPill` do Java para evitar **NPE**), labels do **"Resumo de hoje"** numa linha só (sem `letterSpacing` + `singleLine` + **autosize AppCompat**), **cartão XP** redesenhado (novo `bg_level_badge` + reutilização do `xp_progress_bar`, **ids mantidos**), **banner do versículo** 200→250dp, **atribuir tarefas a amigos/grupos** no Android (`assignees`/`groups` que o backend já aceitava + `FriendApi`/`GroupApi`) e **placeholder "Prioridade"** no spinner (fallback "media" por causa da validação do backend). **Backend inalterado.**
- [**Wordmark, botão "•••" e "X para Y" nas tarefas — 2026-06-27**](android/WORDMARK_DOTS_E_X_PARA_Y_2026-06-27.md) — wordmark "Lifinity" numa linha só no cabeçalho das Tarefas (`maxLines`/`singleLine`), botão de opções "•••" que aparecia como **quadrado verde vazio** (Tarefas e Amigos) corrigido com `ImageView` + vector `ic_dots` (mesmo motivo do FAB: `<Button>`→`MaterialButton`), e nova linha **"criador para destinatário"** nos cartões de tarefa (campos `creator_username`/`assignee_names`/`group_names` já vindos do backend; escondida nas tarefas pessoais).
